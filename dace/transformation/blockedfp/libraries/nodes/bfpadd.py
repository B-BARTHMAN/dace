...
@dace.program
def bfpadd(a_scale: dace.float64[1], a_bias: dace.float64[1], a_fp: dace.float32[16], b_scale: dace.float64[1], b_bias: dace.float64[1], b_fp: dace.float32[16], out_scale: dace.float64[1], out_bias: dace.float64[1], out_fp: dace.float32[16]):
    # Dequantize
...
class ExpandBFPAddNode(ExpandTransformation):
    ...
    @staticmethod
    def expansion(node: "BFPAddNode", parent_state: dace.SDFGState, parent_sdfg: dace.SDFGState) -> dace.nodes.NestedSDFG:
